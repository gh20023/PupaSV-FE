...
document.addEventListener('DOMContentLoaded', () => {
    ...
    btnConfirmar.onclick = async function (e) {
        ...
        try {
            // 1. Obtener el total del carrito ANTES de crear la orden
            const carrito = await carritoApi.obtenerCarrito();
            const monto = carrito.total || 0;

            // 2. Crear la orden
            const orden = await ordenApi.crearOrden(sucursal);

            // 3. Armar el request de pago usando el mapper
            ...
            await pagoApi.realizarPago(pagoRequest);
